...
import { getAttempts, deleteAttemptById } from 'apiSdk/attempts';
import { AttemptInterface } from 'interfaces/attempt';

type ColumnType = ColumnDef<AttemptInterface, unknown>;

interface AttemptListPageProps {
...
export function AttemptListPage(props: AttemptListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AttemptInterface>>(
    () => `/attempts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AttemptInterface) => {
    if (hasAccess('attempt', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/attempts/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/question-papers/view/${record.question_paper?.id}`}
            >
              {record.question_paper?.name}
            </Link>
          ),
        }
      : null,
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/attempts/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('attempt', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/attempts/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('attempt', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="attempt" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Attempts
        </Text>
        {hasAccess('attempt', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/attempts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
